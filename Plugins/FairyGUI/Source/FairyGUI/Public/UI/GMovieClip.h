...
public:
	...
	// from start to end(-1 means ending) repeat times(0 means infinite loop) when all is over, stopping at endAt(-1
	// means same value of end)
	void SetPlaySettings(int32 InStart = 0, int32 InEnd = -1, int32 InTimes = 0, int32 InEndAt = -1,
		const FSimpleDelegate& InCompleteCallback = FSimpleDelegate());

	UFUNCTION(BlueprintCallable, Category = "FairyGUI", meta = (AutoCreateRefTerm = "InCompleteCallback"))
	void SetPlaySettings(const FSimpleDynDelegate& InCompleteCallback, int32 InStart = 0, int32 InEnd = -1,
		int32 InTimes = 0, int32 InEndAt = -1)
	{
		SetPlaySettings(InStart, InEnd, InTimes, InEndAt,
			InCompleteCallback.IsBound()
				? FSimpleDelegate::CreateUFunction(
					  const_cast<UObject*>(InCompleteCallback.GetUObject()), InCompleteCallback.GetFunctionName())
				: FSimpleDelegate());
	}
