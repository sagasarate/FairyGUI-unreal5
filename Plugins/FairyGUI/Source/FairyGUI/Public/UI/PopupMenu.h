...
public:
    ...
    UGButton* AddItem(const FText& Caption, FGUIEventDelegate Callback);

    UFUNCTION(BlueprintCallable, Category = "FairyGUI", meta = (AutoCreateRefTerm = "Callback"))
    UGButton* AddItem(const FText& Caption, const FGUIEventDynDelegate& Callback);

    UGButton* AddItemAt(const FText& Caption, int32 index, FGUIEventDelegate Callback);

    UFUNCTION(BlueprintCallable, Category = "FairyGUI", meta = (AutoCreateRefTerm = "Callback"))
    UGButton* AddItemAt(const FText& Caption, int32 index, const FGUIEventDynDelegate& Callback);

    UFUNCTION(BlueprintCallable, Category = "FairyGUI")
    void AddSeperator();

    void AddSeperator(const FString& ResourceURL);

    UFUNCTION(BlueprintCallable, Category = "FairyGUI")
    const FString& GetItemName(int32 Index) const;

    UFUNCTION(BlueprintCallable, Category = "FairyGUI")
    void SetItemText(const FString& Name, const FText& Caption);
    ...
    UFUNCTION(BlueprintCallable, Category = "FairyGUI")
    UGComponent* GetContentPane() const;
    UFUNCTION(BlueprintCallable, Category = "FairyGUI")
    UGList* GetList() const;
