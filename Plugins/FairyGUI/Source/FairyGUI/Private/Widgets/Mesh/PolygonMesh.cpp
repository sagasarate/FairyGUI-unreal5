...
void FPolygonMesh::BuildMesh(FSubMesh* pSubMesh, float LineWidth, const FColor& LineColor,
	const TArray<FVector2D>& Points, bool bUsePercentPositions, TArray<FColor>* pColors, TArray<FVector2D>* pTexcoords)
{
	...
	const FBox2D& ContentRect = pSubMesh->ContentRect;
	const FBox2D& UVRect = pSubMesh->UVRect;
	int32		  StartIndex = pSubMesh->Vertices.Num();

	FVector2D Size = ContentRect.GetSize();
	bool	  useTexcoords = pTexcoords && pTexcoords->Num() >= numVertices;
	for (int32 i = 0; i < numVertices; i++)
	{
		...
		if (useTexcoords)
		{
			FVector2D uv = FMath::Lerp(UVRect.Min, UVRect.Max, (*pTexcoords)[i]);
			pSubMesh->AddVertex(vec, Color, uv);
		}
		else
	...
	pSubMesh->AddTriangle(RestIndices[0], RestIndices[1], RestIndices[2]);

	if (pColors)
		pSubMesh->RepeatColors(pColors->GetData(), pColors->Num(), StartIndex, numVertices);

	if (LineWidth > 0)
		DrawOutline(pSubMesh, LineWidth, LineColor, Points);
}

void FPolygonMesh::DrawOutline(
	FSubMesh* pSubMesh, float LineWidth, const FColor& LineColor, const TArray<FVector2D>& Points)
{
	int32 numVertices = Points.Num();
	auto& Vertices = pSubMesh->Vertices;
	int32 k = Vertices.Num();
	int32 start = k - numVertices;
	for (int32 i = 0; i < numVertices; i++)
	{
		const FVector2D& p0 = FVector2D(Vertices[start + i].Position);
		FVector2D		 p1;
		if (i < numVertices - 1)
			p1 = FVector2D(Vertices[start + i + 1].Position);
		else
			p1 = FVector2D(Vertices[start].Position);

		FVector2D widthVector(p1.Y - p0.Y, p0.X - p1.X);
		widthVector.Normalize();

		pSubMesh->AddVertex(p0 - widthVector * LineWidth * 0.5f, LineColor);
		pSubMesh->AddVertex(p0 + widthVector * LineWidth * 0.5f, LineColor);
		pSubMesh->AddVertex(p1 - widthVector * LineWidth * 0.5f, LineColor);
		pSubMesh->AddVertex(p1 + widthVector * LineWidth * 0.5f, LineColor);

		k += 4;
		pSubMesh->AddTriangle(k - 4, k - 3, k - 1);
		pSubMesh->AddTriangle(k - 4, k - 1, k - 2);

		// joint
		if (i != 0)
		{
			pSubMesh->AddTriangle(k - 6, k - 5, k - 3);
			pSubMesh->AddTriangle(k - 6, k - 3, k - 4);
		}
		if (i == numVertices - 1)
		{
			start += numVertices;
			pSubMesh->AddTriangle(k - 2, k - 1, start + 1);
			pSubMesh->AddTriangle(k - 2, start + 1, start);
		}
	}
}
...
bool FPolygonMesh::HitTest(
	const FBox2D& ContentRect, const FVector2D& LocalPoint, const TArray<FVector2D>& Points, bool bUsePercentPositions)
{
