...
const FName UPopupMenu::ClickMenu(TEXT("ClickMenu"));
...
void UPopupMenu::Create(const FString& ResourceURL)
{
	...
	List = ContentPane->GetChild(TEXT("list"))->As<UGList>();
...
UGButton* UPopupMenu::AddItem(const FText& Caption, FGUIEventDelegate Callback)
{
	UGButton* item = List->AddItemFromPool()->As<UGButton>();
	item->SetTitle(Caption);
	item->SetGrayed(false);
	UGController* c = item->GetController(TEXT("checked"));
	if (c != nullptr)
...
UGButton* UPopupMenu::AddItem(const FText& Caption, const FGUIEventDynDelegate& Callback)
{
	return AddItem(Caption, Callback.IsBound() ? FGUIEventDelegate::CreateUFunction(const_cast<UObject*>(Callback.GetUObject()), Callback.GetFunctionName()) : FGUIEventDelegate());
}

UGButton* UPopupMenu::AddItemAt(const FText& Caption, int32 Index, FGUIEventDelegate Callback)
{
...
UGButton* UPopupMenu::AddItemAt(const FText& Caption, int32 index, const FGUIEventDynDelegate& Callback)
{
	return AddItemAt(Caption, index, Callback.IsBound() ? FGUIEventDelegate::CreateUFunction(const_cast<UObject*>(Callback.GetUObject()), Callback.GetFunctionName()) : FGUIEventDelegate());
}
...
void UPopupMenu::AddSeperator(const FString& ResourceURL)
{
	List->AddItemFromPool(ResourceURL);
}
...
void UPopupMenu::SetItemText(const FString& Name, const FText& Caption)
{
...
UGComponent* UPopupMenu::GetContentPane() const
{
	return ContentPane;
}

UGList* UPopupMenu::GetList() const
{
	return List;
}