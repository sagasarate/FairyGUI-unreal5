...
UGTree::UGTree() : Indent(30) {}

void UGTree::CreateDisplayObject()
{
...
void UGTree::CreateCell(UGTreeNode* Node)
{
	...
	UGObject* IndentObj = Node->Cell->GetChild(TEXT("indent"));
	if (IndentObj != nullptr)
		IndentObj->SetWidth((Node->Level - 1) * Indent);

	UGController* cc;

	cc = Child->GetController(TEXT("expanded"));
	if (cc != nullptr)
	{
	...
	cc = Child->GetController(TEXT("leaf"));
	if (cc != nullptr)
...
void UGTree::AfterExpanded(UGTreeNode* Node)
{
	...
	UGController* cc = Node->Cell->GetController(TEXT("expanded"));
	if (cc != nullptr)
...
void UGTree::ReadItems(FByteBuffer* Buffer)
{
	...
	for (int32 i = 0; i < cnt; i++)
	{
		...
		SetupItem(Buffer, node->Cell, i);

		Buffer->SetPos(nextPos);
	}
}

UGTreeNode* UGTree::GetRootNode() const
{
	return RootNode;
}

void UGTree::SetTreeNodeRenderer(const FDynTreeNodeRenderer& InDelegate)
{
...
void UGTree::SetOnTreeNodeWillExpand(const FDynOnTreeNodeWillExpand& InDelegate)
{
	if (InDelegate.IsBound())
