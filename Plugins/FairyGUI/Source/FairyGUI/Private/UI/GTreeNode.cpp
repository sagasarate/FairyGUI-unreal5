...
FText UGTreeNode::GetText() const
{
    if (Cell != nullptr)
        return Cell->GetText();
    else
...
void UGTreeNode::SetText(const FText& InText)
{
    if (Cell != nullptr)
...
UGTreeNode* UGTreeNode::GetParent() const
{
    return Parent.Get();
}

UGTree* UGTreeNode::GetTree() const
{
    return Tree.Get();
}

UGComponent* UGTreeNode::GetCell() const
{
    return Cell;
}
