...
UGList::FItemInfo::FItemInfo() : Obj(nullptr), UpdateFlag(0), bSelected(false) {}

UGList::UGList() : bScrollItemToViewOnClick(true), bAutoResizeItem(true), LastSelectedIndex(-1), FirstIndex(-1)
{
	bTrackBounds = true;
}

void UGList::CreateDisplayObject()
{
	Super::CreateDisplayObject();
	SetOpaque(true);
	Pool = MakeShared<FGObjectPool>();
}
...
void UGList::HandleArrowKey(int32 Direction)
{
	...
	switch (Direction)
	{
		...
		case 3: // right
			if (Layout == EListLayoutType::SingleRow || Layout == EListLayoutType::FlowHorizontal
				|| Layout == EListLayoutType::Pagination)
			{
				index++;
...
{
	if (SelectionController != nullptr && !SelectionController->bChanging
		&& Index < SelectionController->GetPageCount())
	{
...
void UGList::ScrollToView(int32 Index, bool bAnimation, bool bSetFirst)
{
	if (bVirtual)
	{
		...
		if (bLoop)
			Index = FMath::FloorToFloat((float)FirstIndex / (float)NumItems) * NumItems + Index;
...
FVector2D UGList::GetSnappingPosition(const FVector2D& InPoint)
{
	if (bVirtual)
	{
		...
		if (Layout == EListLayoutType::SingleColumn || Layout == EListLayoutType::FlowHorizontal)
		{
			...
			if (index < VirtualItems.Num() && InPoint.Y - ret.Y > VirtualItems[index].Size.Y / 2
				&& index < RealNumItems)
		...
		else if (Layout == EListLayoutType::SingleRow || Layout == EListLayoutType::FlowVertical)
		{
			...
			if (index < VirtualItems.Num() && InPoint.X - ret.X > VirtualItems[index].Size.X / 2
				&& index < RealNumItems)
...
void UGList::DoRefreshVirtualList()
{
	...
	if (bLayoutChanged)
	{
		...
		else if (Layout == EListLayoutType::FlowHorizontal)
		{
			...
			{
				CurLineItemCount =
					FMath::FloorToInt((ScrollPane->GetViewSize().X + ColumnGap) / (ItemSize.X + ColumnGap));
		...
		while (HandleScroll1(forceUpdate))
		{
			...
			if (enterCounter > 20)
			{
				UE_LOG(LogFairyGUI, Warning,
					TEXT("list will never be filled as the item renderer function always returns a different size."));
				break;
			}
		}
		HandleArchOrder1();
...
bool UGList::HandleScroll1(bool forceUpdate)
{
			...
			if (forward)
			{
				for (int32 j = reuseIndex; j >= oldFirstIndex; j--)
				{
					...
					if (ii2.Obj != nullptr && ii2.UpdateFlag != ItemInfoVer
						&& ii2.Obj->GetResourceURL().Compare(url) == 0)
					{
	...
	if (curIndex > 0 && NumChildren() > 0 && Container->GetPosition().Y <= 0
		&& GetChildAt(0)->GetY() > -Container->GetPosition().Y)
		return true;
	else
		return false;
}
...
bool UGList::HandleScroll2(bool forceUpdate)
{
	...
	if (curIndex > 0 && NumChildren() > 0 && Container->GetPosition().X <= 0
		&& GetChildAt(0)->GetX() > -Container->GetPosition().X)
		return true;
	else
		return false;
}
...
void UGList::UpdateBounds()
{
	...
	if (Layout == EListLayoutType::SingleColumn)
	{
		...
		ch = curY;
		if (ch <= viewHeight && bAutoResizeItem && ScrollPane != nullptr && ScrollPane->bDisplayInDemand
			&& ScrollPane->VtScrollBar != nullptr)
		{
	...
	else if (Layout == EListLayoutType::SingleRow)
	{
		...
		cw = curX;
		if (cw <= viewWidth && bAutoResizeItem && ScrollPane != nullptr && ScrollPane->bDisplayInDemand
			&& ScrollPane->HzScrollBar != nullptr)
		{
	...
	else if (Layout == EListLayoutType::FlowHorizontal)
	{
			...
			for (i = 0; i < cnt; i++)
			{
				...
				if (j == ColumnCount || i == cnt - 1)
				{
					...
					for (j = lineStart; j <= i; j++)
					{
						...
						{
							child->SetSize(
								FVector2D(child->SourceSize.X + round(child->SourceSize.X * ratio), child->GetHeight()),
								true);
		...
		else
		{
			for (i = 0; i < cnt; i++)
			{
				...
				if ((ColumnCount != 0 && j >= ColumnCount)
					|| (ColumnCount == 0 && curX + child->GetWidth() > viewWidth && maxHeight != 0))
				{
					curX = 0;
					curY += FMath::CeilToFloat(maxHeight) + LineGap;
					maxHeight = 0;
	...
	else if (Layout == EListLayoutType::FlowVertical)
	{
			...
			for (i = 0; i < cnt; i++)
			{
				...
				if (j == LineCount || i == cnt - 1)
				{
					...
					for (j = lineStart; j <= i; j++)
					{
						...
						if (j < i)
						{
							child->SetSize(FVector2D(child->GetWidth(),
											   child->SourceSize.Y + FMath::RoundToFloat(child->SourceSize.Y * ratio)),
								true);
		...
		else
		{
			for (i = 0; i < cnt; i++)
			{
				...
				if ((LineCount != 0 && j >= LineCount)
					|| (LineCount == 0 && curY + child->GetHeight() > viewHeight && maxWidth != 0))
				{
					curY = 0;
					curX += FMath::CeilToFloat(maxWidth) + ColumnGap;
					maxWidth = 0;
	...
	else // Pagination
	{
		...
		if (bAutoResizeItem && ColumnCount > 0)
		{
			...
			for (i = 0; i < cnt; i++)
			{
				...
				if (j == 0
					&& ((LineCount != 0 && k >= LineCount)
						|| (LineCount == 0 && curY + (LineCount > 0 ? eachHeight : child->GetHeight()) > viewHeight)))
				{
					page++;
					curY = 0;
					k = 0;
				...
				if (j == ColumnCount || i == cnt - 1)
				{
					...
					for (j = lineStart; j <= i; j++)
					{
						...
						{
							child->SetSize(
								FVector2D(child->SourceSize.X + FMath::RoundToFloat(child->SourceSize.X * ratio),
						...
						{
							child->SetSize(
								FVector2D(viewWidth - curX, LineCount > 0 ? eachHeight : child->GetHeight()), true);
						}
						if (child->GetHeight() > maxHeight)
							maxHeight = child->GetHeight();
					}
		...
		else
		{
			for (i = 0; i < cnt; i++)
			{
					...
					|| (ColumnCount == 0 && curX + child->GetWidth() > viewWidth && maxHeight != 0))
				{
					...
					if ((LineCount != 0 && k >= LineCount)
						|| (LineCount == 0 && curY + child->GetHeight() > viewHeight && maxWidth != 0)) // new page
					{
						page++;
						curY = 0;
						k = 0;
...
void UGList::SetupItem(FByteBuffer* Buffer, UGObject* Obj, int32 Index)
{
	const FString* str;
	...
	if ((str = Buffer->ReadSP()) != nullptr)
	{
		...
		auto	txt = ToLocText(*GetPackageName(), *FString::Printf(TEXT("%s_text_li%d"), *Key, Index), **str);
		Obj->SetText(txt);
	}
	...
	{
		FString Key = GetLocalizationKey(ID, UserData.AsString());
		auto	txt = ToLocText(*GetPackageName(), *FString::Printf(TEXT("%s_stext_li%d"), *Key, Index), **str);
		btn->SetSelectedTitle(txt);
	}
			...
			for (int32 i = 0; i < cnt; i++)
			{
				...
				if (obj2 != nullptr)
				{
					if (PropID == EObjectPropID::Text)
					{
						FString Key = GetLocalizationKey(ID, UserData.AsString());
						auto	txt = ToLocText(
							   *GetPackageName(), *FString::Printf(TEXT("%s_li%d_%s"), *Key, Index, *target), *value);
...
const FString& UGList::GetDefaultItem() const
{
	return DefaultItem;
}
...
int32 UGList::GetColumnCount() const
{
	return ColumnCount;
}
...
UGController* UGList::GetSelectionController() const
{
...
void UGList::SetItemRenderer(const FDynListItemRenderer& InItemRenderer)
{
	if (InItemRenderer.IsBound())
		ItemRenderer = FListItemRenderer::CreateUFunction(
			const_cast<UObject*>(InItemRenderer.GetUObject()), InItemRenderer.GetFunctionName());
	else
		ItemRenderer.Unbind();
}
...
void UGList::SetItemProvider(const FDynListItemProvider& InItemProvider)
{
	if (InItemProvider.IsBound())
		ItemProvider = FListItemProvider::CreateUFunction(
			const_cast<UObject*>(InItemProvider.GetUObject()), InItemProvider.GetFunctionName());
	else
		ItemProvider.Unbind();
}
