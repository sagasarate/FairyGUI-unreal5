...
void UScrollPane::Setup(FByteBuffer *Buffer)
{
    ...
    MaskContainer = MakeShared<SContainer>(nullptr);
    ...
    if ((flags & 512) == 0)
        MaskContainer->SetClipToBounds(true);
...
void UScrollPane::HandleSizeChanged()
{
    ...
    if (bDontClipMargin)
    {
        maskRect = maskRect.ShiftBy(FVector2D(-Owner->Margin.Left, -Owner->Margin.Top));
...
float UScrollPane::UpdateTargetAndDuration(float Pos, int32 Axis)
{
    ...
    else
    {
        ...
        {
            FVector2D winSize;
            GetWorld()->GetGameViewport()->GetViewportSize(winSize);
...
void UScrollPane::StartTween(int32 Type)
{
    TweenTime.Set(0, 0);
    Tweening = Type;
    GetWorld()->GetTimerManager().SetTimer(TickTimerHandle,
...
{
    float dt = GetWorld()->GetTimerManager().GetTimerElapsed(TickTimerHandle);
...
void UScrollPane::OnTouchMove(UEventContext *Context)
{
    ...
    float deltaTime = FSlateApplication::Get().GetDeltaTime(); // GetWorld()->GetDeltaSeconds();
    float elapsed = GetWorld()->GetTimeSeconds() - LastMoveTime;
...
UGComponent* UScrollPane::GetHeader() const
{
    return Header;
}

UGComponent* UScrollPane::GetFooter() const
{
    return Footer;
}

UGScrollBar* UScrollPane::GetVtScrollBar() const
{
    return VtScrollBar;
}

UGScrollBar* UScrollPane::GetHzScrollBar() const
{
    return HzScrollBar;
}
...
const FVector2D& UScrollPane::GetContentSize() const
{
    return ContentSize;
}

const FVector2D& UScrollPane::GetViewSize() const
{
    return ViewSize;
}