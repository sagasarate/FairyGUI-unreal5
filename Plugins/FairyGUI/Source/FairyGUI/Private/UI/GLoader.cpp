...
void UGLoader::CreateDisplayObject()
{
	DisplayObject = Container = MakeShared<SContainer>(this);
	Content = MakeShared<SMovieClip>(nullptr);
...
const FString& UGLoader::GetURL() const
{
	return URL;
}
...
void UGLoader::SetTexture(UNTexture* Texture)
{
	if (Texture == nullptr)
		return;
	ClearContent();
...
UNTexture* UGLoader::GetTexture() const
{
...
{
	return Content->GetFlip();
}

void UGLoader::SetFlip(EFlipType InFlip)
{
	Content->SetFlip(InFlip);
}

const FColor& UGLoader::GetColor() const
{
...
void UGLoader::LoadExternal()
{
	...
	UAssetManager::GetStreamableManager().RequestAsyncLoad(
		*SoftObjectPath, FStreamableDelegate::CreateUObject(this, &UGLoader::OnExternalLoaded, URL));
}
