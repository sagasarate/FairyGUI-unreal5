...
UGGroup::UGGroup() : MainGridIndex(-1), MainGridMinSize(10), MainChildIndex(-1) {}

void UGGroup::CreateDisplayObject()
{
	DisplayObject = MakeShared<SDisplayObject>(this);
...
void UGGroup::ResizeChildren(const FVector2D& Delta)
{
	...
	if (!bPercentReady)
	{
		...
		for (int32 i = 0; i < cnt; i++)
		{
			...
			if (TotalSize > 0)
				child->SizePercentInGroup =
					(Layout == EGroupLayoutType::Horizontal ? child->GetWidth() : child->GetHeight()) / TotalSize;
			else
	...
	if (Layout == EGroupLayoutType::Horizontal)
	{
		...
		for (int32 i = 0; i < cnt; i++)
		{
			...
			{
				child->SetSize(FVector2D(FMath::RoundToInt(child->SizePercentInGroup / remainPercent * remainSize),
								   child->RawSize.Y + Delta.Y),
					true);
				remainPercent -= child->SizePercentInGroup;
				remainSize -= child->GetWidth();
			}
	...
	else
	{
		...
		for (int32 i = 0; i < cnt; i++)
		{
			...
			if (!priorHandled || i != MainChildIndex)
			{
				child->SetSize(FVector2D(child->RawSize.X + Delta.X,
								   FMath::RoundToInt(child->SizePercentInGroup / remainPercent * remainSize)),
					true);
				remainPercent -= child->SizePercentInGroup;
				remainSize -= child->GetHeight();
			}
...
EGroupLayoutType UGGroup::GetLayout() const
{
	return Layout;
}

int32 UGGroup::GetColumnGap() const
{
	return ColumnGap;
}

int32 UGGroup::GetLineGap() const
{
	return LineGap;
}

bool UGGroup::IsExcludeInvisibles() const
{
	return bExcludeInvisibles;
}

bool UGGroup::IsAutoSizeDisabled() const
{
	return bAutoSizeDisabled;
}

int32 UGGroup::GetMainGridIndex() const
{
	return MainGridIndex;
}

int32 UGGroup::GetMainGridMinSize() const
{
	return MainGridMinSize;
}
