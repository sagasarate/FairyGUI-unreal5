...
{
	FAtlasSprite()
		: Rect(ForceInit), OriginalSize(ForceInit), Offset(ForceInit), bRotated(false) {}
...
{
	UUIPackageAsset* PackageAsset =
		Cast<UUIPackageAsset>(StaticLoadObject(UUIPackageAsset::StaticClass(), nullptr, InAssetPath));
...
void UUIPackage::RemovePackage(const FString& IDOrName, UObject* WorldContextObject)
{
	...
	if (Pkg != nullptr)
	{
		...
		UGRoot::Get(WorldContextObject)->GetObjectPool()->ClearByPackage(Pkg->ID);
...
void UUIPackage::RemoveAllPackages(UObject* WorldContextObject)
{
	...
	for (int32 i = UUIPackageStatic::Get().PackageList.Num() - 1; i >= 0; --i)
	{
		UUIPackage* Pkg = UUIPackageStatic::Get().PackageList[i];
		Pkg->RefWorlds.Remove(WorldID);
		...
		// 没有任何 World 引用此包了，真正移除
		UUIPackageStatic::Get().PackageList.RemoveAt(i);
		UUIPackageStatic::Get().PackageInstByID.Remove(Pkg->ID);
		UUIPackageStatic::Get().PackageInstByID.Remove(Pkg->AssetPath);
		UUIPackageStatic::Get().PackageInstByName.Remove(Pkg->Name);
	}
...
UGObject* UUIPackage::CreateObject(const FString& PackageName, const FString& ResourceName, UObject* WorldContextObject,
	TSubclassOf<UGObject> ClassType)
{
	UUIPackage* pkg = UUIPackage::GetPackageByName(PackageName);
	if (pkg)
		return pkg->CreateObject(ResourceName, WorldContextObject, ClassType);
	else
		return nullptr;
}

UGObject* UUIPackage::CreateObjectFromURL(
	const FString& URL, UObject* WorldContextObject, TSubclassOf<UGObject> ClassType)
{
	TSharedPtr<FPackageItem> pii = UUIPackage::GetItemByURL(URL);
	if (pii.IsValid())
		return pii->Owner->CreateObject(pii, WorldContextObject, ClassType);
	else
		return nullptr;
}
...
FString UUIPackage::GetItemURL(const FString& PackageName, const FString& ResourceName)
{
	...
	if (pkg != nullptr)
	{
		...
		if (pii.IsValid())
			return TEXT("ui://") + pkg->GetID() + pii->ID;
	}
	return "";
}

UNTexture* UUIPackage::GetImageItem(const FString& PackageName, const FString& ResourceName)
{
	UUIPackage* pkg = GetPackageByName(PackageName);
	if (pkg != nullptr)
	{
		...
		if (pii.IsValid())
		{
			if (pii->Type == EPackageItemType::Image)
				return (UNTexture*)pkg->GetItemAsset(pii);
		}
	}
	return nullptr;
}

UNTexture* UUIPackage::GetImageItemByURL(const FString& URL)
{
	TSharedPtr<FPackageItem> pii = UUIPackage::GetItemByURL(URL);
	if (pii.IsValid() && pii->Owner)
	{
		if (pii->Type == EPackageItemType::Image)
			return (UNTexture*)pii->Owner->GetItemAsset(pii);
	}
	return nullptr;
}

UFontManager* UUIPackage::GetFontManager()
{
	return UUIPackageStatic::Get().FontManager;
}
...
TSharedPtr<FPackageItem> UUIPackage::GetItemByURL(const FString& URL)
{
	...
	int32 pos1;
	if (!URL.FindChar(TEXT('/'), pos1))
		return nullptr;

	int32 pos2 = URL.Find(TEXT("/"), ESearchCase::CaseSensitive, ESearchDir::FromStart, pos1 + 2);
	if (pos2 == -1)
	{
		if (URL.Len() > 13)
		{
...
UGObject* UUIPackage::CreateObject(const FString& ResourceName, UObject* WorldContextObject, TSubclassOf<UGObject> ObjClass)
{
	...
	return CreateObject(item, WorldContextObject, ObjClass);
}

UGObject* UUIPackage::CreateObject(const TSharedPtr<FPackageItem>& Item, UObject* WorldContextObject, TSubclassOf<UGObject> ObjClass)
{
	UGObject* g = UUIObjectFactory::NewObject(Item, UGRoot::Get(WorldContextObject), ObjClass);
	if (g == nullptr)
		return nullptr;
...
bool UUIPackage::RegisterFont(FName FontFace, UObject* Font)
{
	auto UIFont = NewObject<UDynamicFont>(GetFontManager());
	if (!UIFont)
		return false;
	GetFontManager()->RegisterFont(UIFont, FontFace);
	return true;
}
...
void UUIPackage::Load(FByteBuffer* Buffer)
{
	...
	for (int32 i = 0; i < cnt; i++)
	{
		...
		info.Add(TEXT("id"), Buffer->ReadS());
		info.Add(TEXT("name"), Buffer->ReadS());
	...
	for (int32 i = 0; i < cnt; i++)
	{
		...
		switch (pii->Type)
		{
			...
			case EPackageItemType::Component:
			{
				...
				UUIObjectFactory::ResolvePackageItemExtension(pii);
				break;
			}
			...
			{
				FString file = fileName + TEXT("_") + FPaths::GetBaseFilename(pii->File);
				pii->File = path + TEXT("/") + file + TEXT(".") + file;
				break;
			}
...
void UUIPackage::LoadMovieClip(const TSharedPtr<FPackageItem>& Item)
{
	...
	for (int32 i = 0; i < frameCount; i++)
	{
		...
		if (!spriteId.IsEmpty() && (sprite = Sprites.FindRef(spriteId)) != nullptr)
		{
			...
			Frame.Texture->Init(
				(UNTexture*)GetItemAsset(sprite->Atlas), sprite->Rect, sprite->bRotated, Item->Size, FrameRect.Min);
		}
...
{
	// 创建 UBitmapFont 并关联到 UFontManager
	UBitmapFont* BitmapFont = NewObject<UBitmapFont>(GetFontManager());
	if (!BitmapFont)
	{
		UE_LOG(LogFairyGUI, Error, TEXT("LoadFont: failed to create UBitmapFont!"));
		Item->RawData.Reset();
		return;
	}
	Item->BitmapFont = BitmapFont;
	BitmapFont->SetName(FName(*Item->ID));

	FByteBuffer* Buffer = Item->RawData.Get();
	Buffer->Seek(0, 0);

	bool bTTF = Buffer->ReadBool();
	BitmapFont->SetCanTint(Buffer->ReadBool());
	BitmapFont->SetResizable(Buffer->ReadBool());
	BitmapFont->SetHasChannel(Buffer->ReadBool());

	int32 FontSize = Buffer->ReadInt();
	BitmapFont->SetSize(FontSize);
	...
	if (bTTF && (MainSprite = Sprites.FindRef(Item->ID)) != nullptr)
		BitmapFont->SetTexture((UNTexture*)GetItemAsset(MainSprite->Atlas));
	...
	for (int32 i = 0; i < cnt; i++)
	{
		...
		if (bTTF)
		{
			...
			Glyph.UVRect = FBox2D(TexCoords / BitmapFont->GetTexture()->GetSize(),
				(TexCoords + GlyphSize) / BitmapFont->GetTexture()->GetSize());
		...
		else
		{
			...
			if (CharImg.IsValid())
			{
				...
				auto OrginSize = CharImg->Size;
				CharImg = CharImg->GetHighResolution();
				GetItemAsset(CharImg);
				Glyph.UVRect = CharImg->Texture->UVRect;

				FVector2D TexScale = OrginSize / CharImg->Size;

				Glyph.Offset = GlyphOffset + CharImg->Texture->Offset * TexScale;
				Glyph.Size = CharImg->Texture->GetSize() * TexScale;

				if (BitmapFont->GetTexture() == nullptr)
					BitmapFont->SetTexture(CharImg->Texture->Root);
			}

			if (BitmapFont->GetSize() == 0)
				BitmapFont->SetSize((int32)GlyphSize.Y);
			...
			if (Glyph.LineHeight < BitmapFont->GetSize())
				Glyph.LineHeight = BitmapFont->GetSize();
		}

		BitmapFont->AddChar(ch, Glyph);
		Buffer->SetPos(nextPos);
	}

	// 注册到 FontManager
	GetFontManager()->RegisterFont(BitmapFont);
...
UUIPackageStatic::UUIPackageStatic()
{
	FontManager = CreateDefaultSubobject<UFontManager>(TEXT("FontManager"));
}
...
UUIPackageStatic& UUIPackageStatic::Get()
{
	...
	{
		Singleton = NewObject<UUIPackageStatic>(GetTransientPackage(), NAME_None, RF_Transient);
		Singleton->AddToRoot();
	}
	return *Singleton;
}
...
void UUIPackageStatic::Destroy()
{
	if (Singleton != nullptr)
	{
		// 清空 FontManager（归还字体 Glyph 引用）
		if (Singleton->FontManager)
		{
			Singleton->FontManager->Clear();
		}

		Singleton->RemoveFromRoot();
		Singleton->MarkAsGarbage();
		Singleton = nullptr;
	}
}

const FString& UUIPackage::GetID() const
{
	return ID;
}

const FString& UUIPackage::GetName() const
{
	return Name;
}
