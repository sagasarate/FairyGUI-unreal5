...
FTransitionItem::FTransitionItem(ETransitionActionType InType)
	: Time(0), Type(InType), Tweener(nullptr), Target(nullptr), DisplayLockToken(0)
{
	switch (InType)
	{
...
UTransition::UTransition()
	: TotalTimes(0), TotalTasks(0), bPlaying(false), bPaused(false), Options(0), bReversed(false), TotalDuration(0), bAutoPlay(false), AutoPlayDelay(0), TimeScale(1), StartTime(0), EndTime(0)
{
}
...
void UTransition::StopItem(FTransitionItem* item, bool bSetToComplete)
{
	...
	if (item->Type == ETransitionActionType::Transition)
	{
		if (item->TransData->Instance != nullptr)
			item->TransData->Instance->Stop(bSetToComplete, false);
	}
}
...
void UTransition::SetValue(const FString& InLabel, const TArray<FNVariant>& InValues)
{
	...
	for (auto& item : Items)
	{
		...
		switch (item->Type)
		{
			...
			case ETransitionActionType::Text:
				item->TextData = InValues[0].AsText();
				break;
			case ETransitionActionType::Icon:
				item->StrData = InValues[0].AsString();
				break;
			default:
				break;
		}
...
void UTransition::ApplyValue(FTransitionItem* item)
{
	...
	switch (item->Type)
	{
		...
		case ETransitionActionType::Icon:
			item->Target->SetIcon(item->StrData.GetValue());
			break;
		default:
			break;
	}
...
void UTransition::DecodeValue(FTransitionItem* item, FByteBuffer* Buffer, FTransitionItemData* Value)
{
	switch (item->Type)
	{
		...
		case ETransitionActionType::Text:
		{
			...
			{
				item->TextData = G_EMPTY_TEXT;
			}
		}
		break;
		case ETransitionActionType::Icon:
			item->StrData = Buffer->ReadS();
			break;
...
void UTransition::Play(const FSimpleDynDelegate& InCompleteCallback, int32 InTimes, float InDelay, float InStartTime, float InEndTime)
{
...
void UTransition::PlayReverse(const FSimpleDynDelegate& InCompleteCallback, int32 InTimes, float InDelay)
{
...
void UTransition::SetHook(const FString& InLabel, FSimpleDynDelegate Callback)
{
