...
FText UGLabel::GetTitle() const
{
	return GetText();
}

void UGLabel::SetTitle(const FText& InTitle)
{
	SetText(InTitle);
}

FText UGLabel::GetText() const
{
...
void UGLabel::SetText(const FText& InText)
{
	if (TitleObject != nullptr)
...
void UGLabel::SetupAfterAdd(FByteBuffer* Buffer, int32 BeginPos)
{
	...
	if (Buffer->ReadBool())
	{
		UObject* obj = GetChild(TEXT("title"));
		if (obj != nullptr && obj->IsA<UGTextInput>())
		{
			UGTextInput* input = Cast<UGTextInput>(obj);
			if ((str = Buffer->ReadSP()) != nullptr)
			{
				FString Key = GetLocalizationKey(ID, UserData.AsString());
				auto	txt = ToLocText(*GetPackageName(), *FString::Printf(TEXT("%s_prompt"), *Key), **str);
				input->SetPrompt(txt);
			}
			if ((str = Buffer->ReadSP()) != nullptr)
