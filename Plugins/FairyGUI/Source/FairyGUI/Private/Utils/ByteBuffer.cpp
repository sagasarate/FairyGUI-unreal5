...
FByteBuffer::FByteBuffer(const uint8* InBuffer, int32 InOffset, int32 InLen, bool bInTransferOwnerShip)
	: bLittleEndian(false), Version(0), Buffer(InBuffer), Offset(InOffset), Length(InLen), Position(0), bOwnsBuffer(bInTransferOwnerShip)
{
}
...
void FByteBuffer::SkipString()
{
	int32 len = ReadUshort();
	Skip(len);
}
void FByteBuffer::SkipString(int32 InLen)
{
	Skip(InLen);
}
