...
void UDemoPullToRefresh::OnDemoStart_Implementation()
{
	...
	// FUIObjectFactory::SetExtension(TEXT("ui://PullToRefresh/Header"), UScrollPaneHeader::StaticClass());
	UUIObjectFactory::SetExtensionWithClass(TEXT("ui://PullToRefresh/Header"), UScrollPaneHeader::StaticClass());

	MainView = UUIPackage::CreateObject(TEXT("PullToRefresh"), TEXT("Main"), this)->As<UGComponent>();
	MainView->MakeFullScreen();
	MainView->SetParentToRoot();

	List1 = MainView->GetChild(TEXT("list1"))->As<UGList>();
	...
	List2 = MainView->GetChild(TEXT("list2"))->As<UGList>();
...
void UDemoPullToRefresh::OnDemoEnd_Implementation()
{
	...
	UUIPackage::RemovePackage(TEXT("PullToRefresh"), this);
}

void UDemoPullToRefresh::RenderListItem1(int32 Index, UGObject* Obj)
{
	Obj->SetText(FText::Format(FText::AsCultureInvariant(TEXT("Item {0}")), List1->GetNumItems() - Index - 1));
}

void UDemoPullToRefresh::RenderListItem2(int32 Index, UGObject* Obj)
{
	Obj->SetText(FText::Format(FText::AsCultureInvariant(TEXT("Item ")), Index));
}
...
void UDemoPullToRefresh::OnPullUpToRefreshHandler(UEventContext* Context)
{
	...
	footer->GetController(TEXT("c1"))->SetSelectedIndex(1);
...
void UDemoPullToRefresh::AddMoreItems2()
{
	...
	footer->GetController(TEXT("c1"))->SetSelectedIndex(0);
