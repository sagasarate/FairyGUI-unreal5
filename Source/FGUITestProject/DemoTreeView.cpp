...
void UDemoTreeView::OnDemoStart_Implementation()
{
    ...
    MainView = UUIPackage::CreateObject(TEXT("TreeView"), TEXT("Main"), this)->As<UGComponent>();
    MainView->MakeFullScreen();
    MainView->SetParentToRoot();

    Tree1 = MainView->GetChild(TEXT("tree"))->As<UGTree>();
    Tree1->On(FUIEvents::ClickItem).AddUObject(this, &UDemoTreeView::OnClickNode);

    Tree2 = MainView->GetChild(TEXT("tree2"))->As<UGTree>();
    ...
    TopNode->UserData = FString(TEXT("I'm a top node"));
    Tree2->GetRootNode()->AddChild(TopNode);
    for (int32 i = 0; i < 5; i++)
    {
        UGTreeNode* Node = UGTreeNode::CreateNode();
        Node->UserData = FString(TEXT("Hello ") + FString::FromInt(i));
        TopNode->AddChild(Node);
    }

    UGTreeNode* AFolderNode = UGTreeNode::CreateNode(true);
    AFolderNode->UserData = FString(TEXT("A folder node"));
    TopNode->AddChild(AFolderNode);
    for (int32 i = 0; i < 5; i++)
    {
        UGTreeNode* Node = UGTreeNode::CreateNode();
        Node->UserData = FString(TEXT("Good ") + FString::FromInt(i));
...
void UDemoTreeView::RenderTreeNode(UGTreeNode* Node, UGComponent* Obj)
{
    if (Node->IsFolder())
    {
        Obj->SetText(Node->UserData.AsText());
    }
    else
    {
        if (Node == SpecialNode)
            Obj->SetIcon(TEXT("ui://TreeView/heart"));
        else
            Obj->SetIcon(TEXT("ui://TreeView/file"));
        Obj->SetText(Node->UserData.AsText());
    }
}